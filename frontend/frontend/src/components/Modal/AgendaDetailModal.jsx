...
const AgendaDetailModal = ({ isOpen, onClose, agendaId, agendaType }) => {
  ...
  const fetchAgendaDetail = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:4000/api/agenda/${agendaId}`, {
        headers: {
      ...
      <div style={{ display: "grid", gap: "16px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
          <div>
            ...
            <div style={{ padding: "8px 12px", background: "#f9fafb", borderRadius: "6px", fontSize: "14px" }}>
              {formatDate(details.undangan_tanggal_acara)}
            </div>
          </div>
          <div>
            <label style={{ fontSize: "14px", fontWeight: "600", color: "#374151", display: "block", marginBottom: "4px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
          ...
          <div>
            ...
            <div style={{ padding: "8px 12px", background: "#f9fafb", borderRadius: "6px", fontSize: "14px" }}>
              {details.rsvp_required === "ya" ? "Ya" : details.rsvp_required === "tidak" ? "Tidak" : "-"}
            </div>
          </div>
        </div>
      ...
      <div style={{ display: "grid", gap: "16px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
          <div>
            ...
            <div style={{ padding: "8px 12px", background: "#f9fafb", borderRadius: "6px", fontSize: "14px" }}>
              {formatDate(details.audiensi_tanggal)}
            </div>
          </div>
          <div>
            <label style={{ fontSize: "14px", fontWeight: "600", color: "#374151", display: "block", marginBottom: "4px" }}>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              <span style={{ fontSize: "24px" }}>{getTypeIcon(agendaType)}</span>
              <h2 style={{ margin: 0, fontSize: "20px", fontWeight: "700", color: "#1f2937" }}>
                Detail {agendaType === "undangan" ? "Undangan" : agendaType === "audiensi" ? "Audiensi" : "Agenda"}
              </h2>
              <span
                style={{
          ...
          {loading ? (
            ...
            <div>
              ...
              <div>
                ...
                {agendaType === "undangan" ? renderUndanganDetails(agendaDetail) : renderAudiensiDetails(agendaDetail)}
              </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  <span
                    style={{
                      ...
                      fontSize: "12px",
                      fontWeight: "600",
                      background: agendaDetail.status_kehadiran === "hadir" ? "#10b981" : 
                                agendaDetail.status_kehadiran === "tidak_hadir" ? "#ef4444" : "#f59e0b",
                      color: "white",
                    }}
                  >
                    {agendaDetail.status_kehadiran === "hadir" ? "Hadir" : 
                     agendaDetail.status_kehadiran === "tidak_hadir" ? "Tidak Hadir" : "Belum Konfirmasi"}
                  </span>
                </div>
